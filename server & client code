...
hum.Died:Connect(function() --// if player dies reset values to default
	Value = "None"
	Equipped = false
	...
	c = plr.Character or plr.CharacterAdded:Wait()
	task.wait(.1)
	hum = c:WaitForChild("Humanoid")
end)
Tool.Equipped:Connect(function() --// equip  and uneqip
	Equipped = true
	script.Fire:FireServer("E")
end)

Tool.Unequipped:Connect(function()
	Equipped = false
	script.Fire:FireServer(Value)
	Value = "None"
	script.Fire:FireServer("UE")
end)


UIS.InputBegan:Connect(function(inp,ty)
	if ty then return end
	local plr = game:GetService("Players").LocalPlayer 
	local Rep = game:GetService("ReplicatedStorage")
	local PD = Rep:WaitForChild("PlayerData")
	local Data = PD:WaitForChild(plr.Name.."_Id")
	local c = plr.Character or plr.CharacterAdded:Wait()
	local hum = c:WaitForChild("Humanoid")
	local mouse = plr:GetMouse()
	
	if Data:FindFirstChild("A") or Data:FindFirstChild("Surfin") or Data:FindFirstChild("Spearin") or Data:FindFirstChild("IEx") or Data:FindFirstChild("S") then return end
    if hum.Health <= 0 then return end
    --// if player is stunned or smth then dont let stop
    
	if inp.KeyCode == Enum.KeyCode.Z and Equipped == true and Value == "None" and plr.Cooldowns:FindFirstChild("Ice surf") == nil then --// checks 
		script.Fire:FireServer("Z") --// start surfing
		Value = "SurfR"
		local hp = hum
		local maxhealth = hp.MaxHealth
		local health = hp.Health
		local Velocity = math.clamp(health/maxhealth * 100,17.5,100)
		local DownTime = 3.25 --// max hold time
		local MHT = .15 --// min hold time (not used here)
		local BodyVelo = Instance.new("BodyVelocity",c.HumanoidRootPart)
		BodyVelo.MaxForce = Vector3.new(10000,10000,10000)
		BodyVelo.Velocity = c.HumanoidRootPart.CFrame.LookVector * Velocity
		BodyVelo.P = 25000
		
	
		while Value == "SurfR" and DownTime > 0 do
			local maxhealth = hp.MaxHealth
			local health = hp.Health
			local Velocity = health/maxhealth * 70
			local Velocity = math.clamp(health/maxhealth * 70,17.5,70) --// reduce speed based on hp
			DownTime -= .05
			BodyVelo.Velocity = c.HumanoidRootPart.CFrame.LookVector * Velocity --// apply speed each tick
			task.wait(.05)
        end
        
        --// end surf
        
		script.Fire:FireServer("SurfR")
		BodyVelo:Destroy()
		
	elseif inp.KeyCode == Enum.KeyCode.X and Equipped == true and Value == "None" and plr.Cooldowns:FindFirstChild("Ice spear") == nil then --// checks n stuff 
        script.Fire:FireServer("X") --// start function
		...
		local BV = Instance.new("BodyGyro",c.HumanoidRootPart)
		BV.MaxTorque = Vector3.new(25000,25000,25000)
		BV.CFrame = CFrame.new(c.HumanoidRootPart.Position,mouse.hit.Position)
		BV.P = 25000

		while Value == "SpearX" and DownTime > 0 or MHT > 0 do
			DownTime -= .05
			MHT -= .05
			BV.CFrame = CFrame.new(c.HumanoidRootPart.Position,mouse.hit.Position)
			task.wait(.05)
		...
		script.Fire:FireServer("SpearX") --// end function and clean up values
		Value = "None"
		BV:Destroy()
		BP:Destroy()
    elseif inp.KeyCode == Enum.KeyCode.C and Equipped == true and Value == "None" and plr.Cooldowns:FindFirstChild("Ice spikes") == nil then
        --// fire spikes
		Value = "Spikes"
		script.Fire:FireServer("C")
		task.wait(1.15) --// debounce
		Value = "None"
    elseif inp.KeyCode == Enum.KeyCode.V and Equipped == true and Value == "None" and plr.Cooldowns:FindFirstChild("Ice age") == nil then
        --// fire ice age
		Value = "Age"
		script.Fire:FireServer("V")
		task.wait(1) --// debounce
		Value = "None"		
	end
end)

UIS.InputEnded:Connect(function(inp,ty)
	if ty then return end
	if inp.KeyCode == Enum.KeyCode.Z and Value == "SurfR" then --// if surfing stop surfing
		Value = "None"
		script.Fire:FireServer("SurfR")
	elseif inp.KeyCode == Enum.KeyCode.X and Value == "SpearX" then --// if holding spear stop spearing
		Value = "None"
		script.Fire:FireServer("SpearX")
	end
end)
